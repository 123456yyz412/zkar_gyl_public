...
def gradient_distribution():
    ...
    available_df = residual_inventory_unsalable()
    for index, row in available_df.iterrows():
        ...
        value_ordered_list =gradient_allocation(len(principle_list),row['residual_inventory'])
...
def residual_inventory_sale_targets():
    """
    计算可用库存：当前可以分配的销售目标 = 销售目标 减 所有负责人动态权重分配到的总和
    :return:
    """
    try:
        with get_mysql_session() as session:
      ...
      ,t.vc_type
			...
			,t.sku_xiao_shou_mu_biao
			,t.total_target
			,(t.sku_xiao_shou_mu_biao - t.total_target) as availible_store
  FROM (
SELECT a.sku
      ,d.vc_type
      ,a.deng_ji
...
WHERE t.sku_xiao_shou_mu_biao - total_target > 0
ORDER BY (t.sku_xiao_shou_mu_biao - t.total_target) DESC,t.deng_ji ASC
            """)
            df = pd.read_sql(stmt, session.connection())
            return df
    except Exception as e:
...
def residual_inventory_unsalable():
    """
    ...
    try:
        with get_mysql_session() as session:
			...
			,t.sku_xiao_shou_mu_biao
			,t.total_target
			,(t.zai_ku_ku_cun - t.total_target - if(t.forecast_values = 0,t.sku_xiao_shou_mu_biao * 4, t.forecast_values * 4)) as residual_inventory 
...
LEFT JOIN model.dim_sku_sales_target_principal d 
	...
	JOIN model.dim_sku_sales_target_principal c 
	  on a.sku = c.sku
ORDER BY a.deng_ji) t 
WHERE t.zai_ku_ku_cun / t.sku_xiao_shou_mu_biao > 5
