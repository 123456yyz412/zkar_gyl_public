...
_engine = create_engine(
    f"mysql+mysqlconnector://u123:u123@8.129.20.246:3306/sods",
    pool_size=5,
    max_overflow=10,
    pool_recycle=3600,
...
def get_page_section(url):
    ...
    PRESET_AGENTS = [
        # 新增1: Tor Browser 13 Windows
        'Mozilla/5.0 (Windows NT 10.0; rv:124.0) Gecko/20100101 Firefox/124.0',  # Tor浏览器通常隐藏具体版本

        # 新增2: Huawei Browser 23 (Android 14)
        'Mozilla/5.0 (Linux; Android 14; LYA-AL00) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36 HuaweiBrowser/23.0.0.0',

        # 新增3: Firefox 124 Ubuntu Desktop
...
def get_item_links():
  ...
  FROM crawl_ebay_pitmanarm_listing1 t 
 WHERE (t.d_num1 LIKE '%sold%') OR (t.d_num2 LIKE '%sold%')) a 
 LEFT JOIN crawl_ebay_pitmanarm_listing2 b 
...
def truncate_crawl_ebay_pitmanarm_listing2():
    try:
        with get_mysql_session() as session:
            # 查询所有item_id
            query_item_links ="""TRUNCATE TABLE crawl_ebay_pitmanarm_listing2"""
            result = session.execute(text(query_item_links))
    except Exception as e:
        print(f"Error: {e}")
def insert_item_specifics_to_mysql(flag:bool):
    # 新增失败记录文件路径
    FAILED_LINKS_FILE = r'D:\迅雷下载\failed_links_pitmanarm.txt'

    try:
        # 清空表
        if flag:
            truncate_crawl_ebay_pitmanarm_listing2()
            # 新增：创建/清空失败记录文件
        ...
        for item_link in item_links_list:
            ...
            if html_str:
                ...
                with get_mysql_session() as session:
                    # 查询item_id
                    stmt = text("""
                        INSERT INTO crawl_ebay_pitmanarm_listing2
