...
def get_page_section(url):
    ...
    PRESET_AGENTS = [
        ...
        # Opera 109 Linux
        'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 OPR/109.0.0.0',
...
def get_item_links():
  ...
  FROM crawl_ebay_brakekit_listing1 t 
 WHERE (t.d_num1 LIKE '%sold%') OR (t.d_num2 LIKE '%sold%')) a 
 LEFT JOIN crawl_ebay_brakekit_listing2 b 
...
def truncate_crawl_ebay_brakekit_listing2():
    try:
        with get_mysql_session() as session:
            # 查询所有item_id
            query_item_links ="""TRUNCATE TABLE crawl_ebay_brakekit_listing2"""
            result = session.execute(text(query_item_links))
    except Exception as e:
        print(f"Error: {e}")
def insert_item_specifics_to_mysql(flag:bool):
    # 新增失败记录文件路径
    FAILED_LINKS_FILE = r'D:\Desktop\yyz\销售模型和目标划分\failed_links_brakekit.txt'

    try:
        # 清空表
        if flag:
            truncate_crawl_ebay_brakekit_listing2()
            # 新增：创建/清空失败记录文件
        ...
        for item_link in item_links_list:
            ...
            if html_str:
                ...
                with get_mysql_session() as session:
                    # 查询item_id
                    stmt = text("""
                        INSERT INTO crawl_ebay_brakekit_listing2
